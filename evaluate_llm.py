...
def load_model_and_tokenizer(
    ...
    base_model: str | Path | None
        HF repo ID or local directory of the full model weights.
    model_path: str | Path | None
        HF repo ID or local directory containing **LoRA adapter weights**.
    args: Namespace | Any
        Object providing CLI‑style flags (see ArgsNamespace below).
    """

    if base_model is None and model_path is None:
        raise ValueError("Provide at least `base_model` or `model_path`.")

    args = args or ArgsNamespace()  # use defaults if none supplied
    device = get_device(args)
    print(f"[loader] Using device: {device}")

    fp16, bf16 = determine_model_precision(args)
    dtype = torch.bfloat16 if bf16 else torch.float16
    print(f"[loader] Compute dtype: {dtype}")
