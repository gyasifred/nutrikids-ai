...
def plot_factor_analysis(factors_df, malnutrition_status, factor_combinations=None):
    ...
    sns.barplot(x='correlation', y=stats_df.index,
                data=stats_df.reset_index(), palette='viridis')
    ...
    sns.barplot(x='index', y='prevalence', hue='status', data=comp_df,
                palette='Set2')
    ...
    if factor_combinations is not None and not factor_combinations.empty:
        ...
        sns.barplot(x='lift', y='combination', hue='num_factors',
                    data=top, palette='viridis')
        plt.tight_layout()
...
def analyze_prediction_types(data_dict):
    ...
    plt.figure(figsize=(14,10))
    melted = pd.melt(all_prev.reset_index(), id_vars=['index'],
                     var_name='prediction_type', value_name='prevalence')
