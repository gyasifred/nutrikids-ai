...
def predict_tabpfn(
    ...
    if text_input is not None:
        ...
        if text_column is None:
            raise ValueError("text_column must be provided when using an input_source file")
        
        print(f"Processing input file: {input_source}")
        input_df = pd.read_csv(input_source)
        
        if text_column not in input_df.columns:
            raise ValueError(f"Text column '{text_column}' not found in input file. Available columns: {list(input_df.columns)}")
    ...
    if full_output_dir:
        ...
        plt.ylabel("Count")
        prob_plot_path = os.path.join(full_output_dir, "probability_distribution.png")
        ...
        if calculate_importance and len(X) > 1:  # Need more than one sample for meaningful importance
            ...
            if not importance_df.empty:
                ...
                if importance_fig:
                    importance_plot_path = os.path.join(full_output_dir, "feature_importance.png")
                    importance_fig.savefig(importance_plot_path, bbox_inches='tight', dpi=300)
...
def main():
    ...
    parser.add_argument('--id_column', type=str, help='Name of the column containing IDs')
    
    # Output parameters
    parser.add_argument('--output_dir', type=str, help='Directory to save all prediction artifacts')
