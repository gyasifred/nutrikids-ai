...
def main():
    ...
    label_encoder = None
    # Fix the deprecation warning by using isinstance for categorical type check
    if isinstance(y.dtype, pd.CategoricalDtype) or pd.api.types.is_object_dtype(y) or len(np.unique(y)) < len(y) * 0.5:
        label_encoder = LabelEncoder()
        y = label_encoder.fit_transform(y)
        label_encoder_path = os.path.join(
            args.model_dir,
            f"{args.model_name}_nutrikidai_classifier_label_encoder_{timestamp}.joblib")
