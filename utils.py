...
def format_prompt(patient_notes, few_shot_examples=None):
    """
    Format the prompt for malnutrition classification.

    Args:
        patient_notes (str): The patient's clinical notes
        few_shot_examples (list, optional): List of example cases with notes and classifications

    Returns:
        str: Formatted prompt for the classification model
    """
    instructions = """Read the patient's notes and determine if the patient is likely to have malnutrition. 
Make a definitive classification: malnutrition=yes or malnutrition=no. Use the following criteria:
...
5) Additional risk factors (medications, mental health, socioeconomic status)"""
    ...
    classification_table = """Standard Classification:
...
Explanation: Provide a short reasoning based on the data."""
    ...
    if few_shot_examples and len(few_shot_examples) > 0:
        few_shot_text = "\n\n".join(
            [format_example(example) for example in few_shot_examples])
...
def format_example(example):
    ...
    Args:
        example (dict): Dictionary containing 'notes', 'classification', and 'explanation'

    Returns:
        str: Formatted example
    """
    return (f"Patient Notes: {example['notes']}\n"
            f"Classification: malnutrition={example['classification']}\n"
            f"Explanation: {example['explanation']}")

def parse_classification(model_output):
    """
    Parse the classification result from model output.

    Args:
        model_output (str): The raw output from the classification model

    Returns:
        dict: Dictionary with classification results including decision and explanation
    """
    # Extract classification decision (yes/no)
    decision = None
    if "malnutrition=yes" in model_output.lower():
        decision = "yes"
    elif "malnutrition=no" in model_output.lower():
        decision = "no"
    
    # Extract explanation
    explanation = None
    if "explanation:" in model_output.lower():
        explanation_parts = model_output.lower().split("explanation:")
