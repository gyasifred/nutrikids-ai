...
Can process both raw text input and CSV files, providing explainability for predictions.
"""
...
def get_prediction_explanation(model, X, feature_names, top_n=5):
    ...
    try:
        ...
        if X.shape[0] == 1:
            ...
            explanation = {
                "top_features": [(feature_names[i], float(values[i])) for i in indices],
        ...
        logging.warning(f"SHAP explanation failed: {str(e)}. Falling back to feature importance.")
    ...
    try:
        if hasattr(model, 'feature_importances_'):
            ...
            # Create explanation dictionary
            explanation = {
                "top_features": [(feature_names[i], float(feature_values[i])) for i in indices 
                                if i < len(feature_names) and i < len(feature_values)],
...
def format_explanation(explanation, class_names):
    ...
    if explanation["method"] == "shap":
        ...
        for i, (feature, value) in enumerate(explanation["top_features"]):
            ...
            lines.append(f"{i+1}. {feature}: {direction}{abs(value):.4f} ({'increases' if value > 0 else 'decreases'} prediction)")
...
def plot_explanation(explanation, output_path):
    ...
    plt.xlabel('Contribution to prediction' if explanation["method"] == "shap" else 'Feature importance')
    plt.title('Feature Contributions to Prediction' if explanation["method"] == "shap" else 'Top Important Features')
...
def predict_text(text, xgb_model, pipeline, label_encoder, feature_names, explain=True, top_n=5):
    """
    ...
    if explain:
        explanation = get_prediction_explanation(xgb_model, X_aligned, feature_names, top_n)
        result["explanation"] = explanation
        result["explanation_text"] = format_explanation(explanation, class_names)
    
    return result


def process_csv_file(data_file, text_column, id_column, xgb_model, pipeline, 
                    label_encoder, feature_names, output_dir, explain=True, top_n=5):
    """
    ...
    if text_column not in df.columns:
        raise ValueError(f"Text column '{text_column}' not found in CSV file. Available columns: {list(df.columns)}")
    ...
    for idx, row in df.iterrows():
        ...
        pred_result = predict_text(
            text, xgb_model, pipeline, label_encoder, feature_names, explain, top_n
        )
        ...
        if explain and "explanation" in pred_result:
            ...
            if output_dir:
                os.makedirs(os.path.join(output_dir, "explanation_plots"), exist_ok=True)
                plot_path = os.path.join(output_dir, "explanation_plots", f"explanation_{id_value}.png")
    ...
    if output_dir:
        ...
        if explanations:
            explanations_path = os.path.join(output_dir, "prediction_explanations.txt")
            with open(explanations_path, 'w') as f:
                for expl in explanations:
                    f.write(f"--- ID: {expl['id']} ---\n")
                    f.write(expl["explanation"])
                    f.write("\n\n" + "-" * 80 + "\n\n")
            logging.info(f"Prediction explanations saved to {explanations_path}")
...
def main():
    ...
    data_group.add_argument('--data_file', type=str, help='Path to the CSV test data file')
    data_group.add_argument('--text', type=str, help='Raw text input for prediction')
    ...
    parser.add_argument('--output_dir', type=str, default='./xgb_predictions', help='Directory to save prediction results')
    parser.add_argument('--explain', action='store_true', default=False, help='Generate explanations for predictions')
    parser.add_argument('--top_n_features', type=int, default=5, help='Number of top features to include in explanation')
    ...
    try:
        ...
        if args.text:
            ...
            if args.output_dir:
                ...
                result_path = os.path.join(args.output_dir, "single_prediction_result.txt")
                ...
                if args.explain and "explanation" in result:
                    plot_path = os.path.join(args.output_dir, "single_prediction_explanation.png")
            ...
            print(f"Full results saved to: {os.path.join(args.output_dir, 'prediction_results.csv')}")
            if args.explain:
                print(f"Explanations saved to: {os.path.join(args.output_dir, 'prediction_explanations.txt')}")
                print(f"Explanation plots saved to: {os.path.join(args.output_dir, 'explanation_plots/')}")
